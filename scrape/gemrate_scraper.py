...
_SCRAPE_DIR = Path(__file__).resolve().parent
_REPO_ROOT = Path(__file__).resolve().parents[1]
if str(_SCRAPE_DIR) not in sys.path:
    sys.path.insert(0, str(_SCRAPE_DIR))
from dataset_report_paths import GEMRATE_SCRAPE_SKIPPED_TXT  # noqa: E402


def _default_dataset_path() -> Path:
    """Explorer dataset lives at repo root (same as other sync scripts)."""
    return _REPO_ROOT / "pokemon_sets_data.json"
...
def write_missing_gemrate_report_temp(*, data_path: Path | None = None):
    """Write sets with no GemRate mapping or no card-level pop data to a temp .txt; return path."""
    dp = Path(data_path) if data_path else _default_dataset_path()
    local_sets = _load_local_sets(str(dp))
...
def run_scraper(
    ...
    *,
    ...
    sleep_between_sets_sec: float = 2.0,
):
    dp = Path(data_path) if data_path else _default_dataset_path()
    gem_sets = fetch_gemrate_sets()
    ...
    for lset in local_sets:
        ...
        if not cards_data:
            ...
            time.sleep(max(0.0, sleep_between_sets_sec))
            continue
    ...
    report_path = GEMRATE_SCRAPE_SKIPPED_TXT
    ...
    _atomic_write_json(str(dp), local_sets)
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(description="GemRate Universal Pop → pokemon_sets_data.json (top_25_cards)")
    ap.add_argument(
        "--data",
        "--input",
        type=Path,
        default=None,
        dest="data_path",
        help="Dataset JSON (default: repo root pokemon_sets_data.json)",
    )
    ap.add_argument(
        "--sleep-sets",
        type=float,
        default=2.0,
        help="Seconds to sleep after each set scrape (rate limit GemRate)",
    )
    ap.add_argument(
        "--report-missing",
        "--missing-gemrate-report",
        "--report-missing-gemrate",
        action="store_true",
        dest="report_missing",
        help="Write a temp TSV report path to stdout (no scrape)",
    )
    ap.add_argument(
        "target_set_substrings",
        nargs="*",
        metavar="SUBSTRING",
        help="If set, only process sets whose set_name contains any of these substrings",
    )
    args = ap.parse_args()
    if args.report_missing:
        out = write_missing_gemrate_report_temp(data_path=args.data_path)
        print(out)
    else:
        ts = args.target_set_substrings or None
        run_scraper(
            ts,
            data_path=args.data_path,
            sleep_between_sets_sec=max(0.0, float(args.sleep_sets)),
        )
